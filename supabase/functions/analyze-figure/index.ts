...
const FALLBACK_ASSESSOR_PROMPT = `You are an expert assistant for squares.vote, which uses the CORE framework to map political positions across four dimensions:
  ...
  try {
    ...
    try {
      ...
      const prompt = contextNotes
        ? `Provide a CORE assessment for: ${figureName}\n\nAdditional context: ${contextNotes}`
        : `Provide a CORE assessment for: ${figureName}`;
