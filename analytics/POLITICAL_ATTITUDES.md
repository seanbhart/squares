The CORE Political Typology

Political Attitudes, Not Ideologies

The CORE model maps political outlooks using four cognitive dimensions rather than party or ideology.
It helps people see that disagreements often come from attitudes and worldview differences — not morality or intelligence.

## What is CORE?

**CORE** stands for:
- **C**ivil Rights (Liberty ↔ Authority)
- **O**penness (Global ↔ National)
- **R**edistribution (Market ↔ Social)
- **E**thics (Progressive ↔ Traditional)

The CORE framework is part of **Squares**, a visual political mapping system. Each person or movement gets four colors representing their CORE scores, displayed as square emojis for instant visual recognition.

## Scoring System

Each dimension uses a **0-5 scale**:
- **0-2**: Low end of spectrum (Liberty, Global, Market, Progressive)
- **3-5**: High end of spectrum (Authority, National, Social, Traditional)

### Visual Color Coding
🟪 **0 = Purple** | 🟦 **1 = Blue** | 🟩 **2 = Green** | 🟨 **3 = Yellow** | 🟧 **4 = Orange** | 🟥 **5 = Red**

**Example: LGSP with scores (1, 2, 4, 0)**
- C = 1 (Liberty) → 🟦
- O = 2 (Global) → 🟩
- R = 4 (Social) → 🟧
- E = 0 (Progressive) → 🟪

**Visual: 🟦🟩🟧🟪** represents a progressive globalist with moderate civil liberties preference and a preference for social welfare.

⸻

🧭 The Four CORE Dimensions

Each person answers a set of yes/no toggles.
More YES answers = higher value on that dimension (0–5 scale).

| Dimension | Low (0-2) | High (3-5) | What It Measures |
|-----------|-----------|------------|------------------|
| **Civil Rights (C)** | **Liberty**: favors autonomy, free speech, decentralization | **Authority**: favors coordination, order, rules | Trust in institutions vs individuals |
| **Openness (O)** | **Global**: open borders, trade, cosmopolitan curiosity | **National**: rooted identity, protection of local culture | Comfort with change and difference |
| **Redistribution (R)** | **Market**: individual effort, voluntary exchange | **Social**: collective welfare, safety nets | How we balance autonomy and fairness |
| **Ethics (E)** | **Progressive**: equality, moral flexibility | **Traditional**: hierarchy, moral structure | Preferred moral framework |

### Score Interpretation
- **C: Liberty (0-2) ↔ Authority (3-5)**
- **O: Global (0-2) ↔ National (3-5)**
- **R: Market (0-2) ↔ Social (3-5)**
- **E: Progressive (0-2) ↔ Traditional (3-5)**


⸻

🔘 The Eight Toggle Tags (Simplified Questionnaire)

Each dimension uses two yes/no questions (eight total).
Each “YES” raises that axis score by one point.

1. Civil Liberties (L/A)
	•	✅ Should government have strong powers to regulate harmful or false speech online?
	•	✅ Should police and security agencies have expanded tools for surveillance and data collection?

2. Openness (G/N)
	•	✅ Should immigration be limited to protect national culture and jobs?
	•	✅ Should trade barriers be raised to favor domestic industry?

3. Redistribution (M/S)
	•	✅ Should the government guarantee universal healthcare and a living wage?
	•	✅ Should large companies share profits or decision-making with workers?

4. Ethics (P/T)
	•	✅ Should abortion or same-sex marriage be legally restricted to preserve traditional values?
	•	✅ Should businesses and schools be allowed to follow their religious or cultural rules even if discriminatory?

Add your YES answers → plot the scores (0–5 each).
Combine the letters from your results (e.g., LGSP) to reveal your political attitude.

⸻

🌐 The Four Political Families

Family	Code	Description	Attitude Archetype
Builders	GM (Market Globalists)	Innovative, entrepreneurial, cosmopolitan. See change as opportunity.	“Inventors” – pragmatic optimists who like free systems and open exchange.
Diplomats	GS (Social Globalists)	Cooperative, humanitarian, reformist. Believe global cooperation solves shared problems.	“Healers” – empathetic and civic-minded global citizens.
Unionists	NS (Social Nationalists)	Protective, community-centered, fairness-focused. Advocate strong state and social solidarity.	“Guardians” – rooted egalitarians who defend their own.
Proprietors	NM (Market Nationalists)	Self-reliant, localist, pragmatic. Value independence and community tradition.	“Stewards” – practical caretakers of home and heritage.

Sentinels (N + T overlay) — A cross-family temperament emphasizing order, loyalty, and moral clarity. Includes most Traditional Nationals (NatCons, Postliberals, Fascists).

⸻

🧩 The Sixteen Political Attitudes

Family	Type	Name	Typical Examples
GM – Builders (Market Globalists)
    LGMP	Optimists	Abundance movement, startup libertarians
	AGMP	Architects	Managerial futurists, neoliberal reformers
	LGMT	Libertarians	Classical liberals, liberty caucus (global wing)
	AGMT	Institutionalists	Institutional liberals, moderate conservatives
GS – Diplomats (Social Globalists)
    LGSP	Cooperators	Nordic social democrats, Builder Bloc
	AGSP	Technocrats	Postscarcity progressives, managerial left
	LGST	Humanists	Faith-based global charity left
	AGST	Global Conservatives	Global corporatists, social conservatives abroad
NS – Unionists (Social Nationalists)
    LNSP	Laborists	Democratic-social left, labor populists
	ANSP	Revolutionaries	Communists, revolutionary socialists
	LNST	National Conservatives	Populist pro-life left, communal traditionalists
	ANST	Order Conservatives	NatCon, Postliberal, Fascist hybrids
NM – Proprietors (Market Nationalists)
    LNMP	Merchants	Localist entrepreneurs, small-business conservatives
	ANMP	Modernizers	State-guided modernizers, national capitalists
	LNMT	Stewards	Localist conservatives, community republicans
	ANMT	Enforcers	Hard-order nationalists, militarized right

⸻

🧠 How to Read Your CORE Type

Example: LGSP (Diplomat)
	•	L – Liberty: favors free expression and light policing.
	•	G – Global: open to trade and migration.
	•	S – Social: supports welfare and collective solutions.
	•	P – Progressive: emphasizes equality over tradition.

That person is an empathetic, cooperative global reformer — the Diplomat family.

⸻

📊 Dimensional Importance by Type

Each attitude type has **7 points** distributed across the four CORE dimensions, indicating which dimensions most strongly define that attitude. Each dimension receives at least 1 point.

**Reading the Visual Representations:**
- **Binary**: ⬛️ = less government (Liberty/Global/Market/Progressive), ⬜️ = more government (Authority/National/Social/Traditional)
- **Mid-Value**: Simplified color coding where 🟦 = L/G/M/P (blue for less government) and 🟧 = A/N/S/T (orange for more government)

*Example: AGSP (Technocrats) shows ⬜️⬛️⬜️⬛️ in binary and 🟧🟦🟧🟦 in mid-value, indicating Authority-Global-Social-Progressive preferences.*

### GM – Builders (Market Globalists)

| Type | Name | Binary | Mid-Value | C (Civil) | O (Openness) | R (Redistribution) | E (Ethics) | Key Dimensions |
|------|------|--------|-----------|-----------|--------------|-------------------|------------|----------------|
| **LGMP** | Optimists | ⬛️⬛️⬛️⬛️ | 🟦🟦🟦🟦 | 1 | 1 | 1 | 1 | Balanced across C, O, R |
| **AGMP** | Architects | ⬜️⬛️⬛️⬛️ | 🟧🟦🟦🟦 | 4 | 1 | 1 | 1 | **R** (Market innovation), O |
| **LGMT** | Libertarians | ⬛️⬛️⬛️⬜️ | 🟦🟦🟦🟧 | 1 | 1 | 1 | 4 | **C** (Liberty), O |
| **AGMT** | Institutionalists | ⬜️⬛️⬛️⬜️ | 🟧🟦🟦🟧 | 4 | 1 | 1 | 4 | Balanced institutional focus |

**Intensity Variations:**

**LGMP (Liberty-Global-Market-Progressive) Subtypes:**
- **2222** - **Pragmatists**: Moderate believers in market solutions with balanced compromise
- **1111** - **Optimists** *(default)*: Confident innovators who trust free systems
- **0000** - **Visionaries**: Radical free-market globalists pushing boundaries

**AGMP (Authority-Global-Market-Progressive) Subtypes:**
- **3222** - **Reformers**: Cautious improvers working within systems
- **4111** - **Architects** *(default)*: Strategic builders of market-oriented institutions
- **5000** - **Masterplanners**: Commanding designers of global economic frameworks

**LGMT (Liberty-Global-Market-Traditional) Subtypes:**
- **2223** - **Conventionalists**: Moderate conservatives favoring stability
- **1114** - **Libertarians** *(default)*: Classical liberals defending tradition through freedom
- **0005** - **Constitutionalists**: Strict originalists with absolute property rights

**AGMT (Authority-Global-Market-Traditional) Subtypes:**
- **3223** - **Establishmentarians**: Safe-choice maintainers of status quo
- **4114** - **Institutionalists** *(default)*: Principled defenders of established order
- **5005** - **Guardians**: Firm protectors of traditional market hierarchies

### GS – Diplomats (Social Globalists)

| Type | Name | Binary | Mid-Value | C (Civil) | O (Openness) | R (Redistribution) | E (Ethics) | Key Dimensions |
|------|------|--------|-----------|-----------|--------------|-------------------|------------|----------------|
| **LGSP** | Cooperators | ⬛️⬛️⬜️⬛️ | 🟦🟦🟧🟦 | 1 | 1 | 4 | 1 | **R** (Social welfare), O |
| **AGSP** | Technocrats | ⬜️⬛️⬜️⬛️ | 🟧🟦🟧🟦 | 4 | 1 | 4 | 1 | **R** (Redistribution), O |
| **LGST** | Humanists | ⬛️⬛️⬜️⬜️ | 🟦🟦🟧🟧 | 1 | 1 | 4 | 4 | O, balanced R/E |
| **AGST** | Global Conservatives | ⬜️⬛️⬜️⬜️ | 🟧🟦🟧🟧 | 4 | 1 | 4 | 4 | C, O, E balance |

**Intensity Variations:**

**LGSP (Liberty-Global-Social-Progressive) Subtypes:**
- **2232** - **Facilitators**: Gentle coordinators building consensus step by step
- **1141** - **Cooperators** *(default)*: Collaborative reformers championing social welfare
- **0050** - **Solidarity Leaders**: Uncompromising champions of universal welfare rights

**AGSP (Authority-Global-Social-Progressive) Subtypes:**
- **3232** - **Planners**: Thoughtful policy designers with measured approach
- **4141** - **Technocrats** *(default)*: Expert-driven managers of redistribution systems
- **5050** - **Social Engineers**: Commanding architects of total welfare states

**LGST (Liberty-Global-Social-Traditional) Subtypes:**
- **2233** - **Caregivers**: Gentle servants upholding communal bonds
- **1144** - **Humanists** *(default)*: Faith-based global charity advocates
- **0055** - **Missionaries**: Devoted healers spreading traditional compassion worldwide

**AGST (Authority-Global-Social-Traditional) Subtypes:**
- **3233** - **Corporatists**: Moderate organizers of structured societies
- **4144** - **Global Conservatives** *(default)*: Principled defenders of ordered social hierarchies
- **5055** - **Paternalists**: Firm guides enforcing traditional welfare obligations

### NS – Unionists (Social Nationalists)

| Type | Name | Binary | Mid-Value | C (Civil) | O (Openness) | R (Redistribution) | E (Ethics) | Key Dimensions |
|------|------|--------|-----------|-----------|--------------|-------------------|------------|----------------|
| **LNSP** | Laborists | ⬛️⬜️⬜️⬛️ | 🟦🟧🟧🟦 | 1 | 4 | 4 | 1 | **R** (Social solidarity), O |
| **ANSP** | Revolutionaries | ⬜️⬜️⬜️⬛️ | 🟧🟧🟧🟦 | 4 | 4 | 4 | 1 | **C** (Authority), R |
| **LNST** | National Conservatives | ⬛️⬜️⬜️⬜️ | 🟦🟧🟧🟧 | 1 | 4 | 4 | 4 | O, balanced R/E |
| **ANST** | Order Conservatives | ⬜️⬜️⬜️⬜️ | 🟧🟧🟧🟧 | 4 | 4 | 4 | 4 | **C** (Authority), E |

**Intensity Variations:**

**LNSP (Liberty-National-Social-Progressive) Subtypes:**
- **2332** - **Organizers**: Moderate union supporters seeking gradual reform
- **1441** - **Laborists** *(default)*: Committed workers' advocates demanding social solidarity
- **0550** - **Syndicalists**: Militant labor radicals seizing collective control

**ANSP (Authority-National-Social-Progressive) Subtypes:**
- **3332** - **Collectivists**: Pragmatic statists coordinating national economies
- **4441** - **Revolutionaries** *(default)*: Vanguard leaders overthrowing capitalist systems
- **5550** - **Communists**: Iron-fisted architects of total state socialism

**LNST (Liberty-National-Social-Traditional) Subtypes:**
- **2333** - **Populists**: Folkish advocates balancing tradition and community
- **1444** - **National Conservatives** *(default)*: Rooted defenders of faith and fatherland
- **0555** - **Supremacists**: Fanatical tribalists enforcing blood-and-soil purity

**ANST (Authority-National-Social-Traditional) Subtypes:**
- **3333** - **Disciplinarians**: Stern enforcers maintaining traditional order
- **4444** - **Order Conservatives** *(default)*: Total authoritarians demanding obedience to hierarchy
- **5555** - **Totalitarians**: Absolute dictators crushing all dissent and difference

### NM – Proprietors (Market Nationalists)

| Type | Name | Binary | Mid-Value | C (Civil) | O (Openness) | R (Redistribution) | E (Ethics) | Key Dimensions |
|------|------|--------|-----------|-----------|--------------|-------------------|------------|----------------|
| **LNMP** | Merchants | ⬛️⬜️⬛️⬛️ | 🟦🟧🟦🟦 | 1 | 4 | 1 | 1 | **R** (Market), O |
| **ANMP** | Modernizers | ⬜️⬜️⬛️⬛️ | 🟧🟧🟦🟦 | 4 | 4 | 1 | 1 | **C** (Authority), O |
| **LNMT** | Stewards | ⬛️⬜️⬛️⬜️ | 🟦🟧🟦🟧 | 1 | 4 | 1 | 4 | balanced R/E |
| **ANMT** | Enforcers | ⬜️⬜️⬛️⬜️ | 🟧🟧🟦🟧 | 4 | 4 | 1 | 4 | **C** (Authority), E |

**Intensity Variations:**

**LNMP (Liberty-National-Market-Progressive) Subtypes:**
- **2322** - **Tradesmen**: Practical local business owners favoring stability
- **1411** - **Merchants** *(default)*: Independent entrepreneurs protecting community commerce
- **0500** - **Isolationists**: Hardcore protectionists shutting out foreign competition

**ANMP (Authority-National-Market-Progressive) Subtypes:**
- **3322** - **Nationalists**: Strategic developers building domestic industry
- **4411** - **Modernizers** *(default)*: State-guided capitalists advancing national power
- **5500** - **Autarchs**: Commanding economic nationalists demanding total self-sufficiency

**LNMT (Liberty-National-Market-Traditional) Subtypes:**
- **2323** - **Townspeople**: Neighborly shopkeepers upholding local customs
- **1414** - **Stewards** *(default)*: Faithful caretakers preserving heritage and property
- **0505** - **Separatists**: Rigid traditionalists rejecting outside influence entirely

**ANMT (Authority-National-Market-Traditional) Subtypes:**
- **3323** - **Sentinels**: Watchful defenders of borders and hierarchies
- **4414** - **Enforcers** *(default)*: Hard-order nationalists crushing threats to tradition
- **5505** - **Fascists**: Brutal authoritarians imposing racial/cultural domination

### Key Patterns

**Most Authority-Focused (C=3):**
- LGMT (Libertarians at opposite end - Liberty)
- ANSP (Revolutionaries)
- ANST (Order Conservatives)
- ANMP (Modernizers)
- ANMT (Enforcers)

**Most Openness-Focused (O=2):**
- Nearly all types show moderate Openness importance, as it's the primary divider between Global (Builders/Diplomats) and National (Unionists/Proprietors)

**Most Redistribution-Focused (R=3):**
- AGMP (Architects - market innovation)
- LGSP (Cooperators - social welfare)
- AGSP (Technocrats - redistribution policy)
- LNSP (Laborists - labor solidarity)
- LNMP (Merchants - market freedom)

**Most Ethics-Focused (E=2):**
- LGST, AGST (Humanists/Global Conservatives)
- LNST (National Conservatives)
- ANST, LNMT, ANMT (Traditional nationals)

⸻

💬 The CORE Idea

People differ less in goals than in cognitive temperament:
	•	Builders trust freedom to innovate.
	•	Diplomats trust cooperation to heal.
	•	Unionists trust solidarity to protect.
	•	Proprietors trust community to preserve.

Seeing politics this way turns conflict into conversation —
from “who’s right” to “how do we think differently?”

⸻